...
export function Activity({ username }: { username?: string }) {
  const [activeTab, setactiveTab] = useState<'Posts' | 'Comments' | 'Likes' | 'Saved'>('Posts');
  const userContent = useGetUserContent({
    username,
    types: ['Analysis', 'Blog', 'Signal', 'Tweet'],
    markets: ['Forex'],
    i_bookmarked: activeTab === 'Saved',
    i_commented: activeTab === 'Comments',
    i_liked: activeTab === 'Likes',
    i_reposted: activeTab === 'Posts'
  });
      ...
      <Box component='main'>
        <Box
          ...
          {['Posts', 'Comments'].map((type) => (
            <Button
              onClick={() => setactiveTab(type as 'Posts' | 'Comments')}
              key={type}
              variant='outlined'
              sx={{
                fontSize: '14px',
